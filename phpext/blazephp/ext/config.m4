PHP_ARG_ENABLE(blazephp, whether to enable blazephp, [ --enable-blazephp   Enable Blazephp])

if test "$PHP_BLAZEPHP" = "yes"; then

	

	if ! test "x" = "x"; then
		PHP_EVAL_LIBLINE(, BLAZEPHP_SHARED_LIBADD)
	fi

	AC_DEFINE(HAVE_BLAZEPHP, 1, [Whether you have Blazephp])
	...
	AC_CHECK_DECL(
		...
		[
			AC_CHECK_HEADERS(
				[ext/pcre/php_pcre.h],
				[
					PHP_ADD_EXTENSION_DEP([blazephp], [pcre])
	...
	AC_CHECK_DECL(
		...
		[
			AC_CHECK_HEADERS(
				[ext/json/php_json.h],
				[
					PHP_ADD_EXTENSION_DEP([blazephp], [json])
	...
	PHP_INSTALL_HEADERS([ext/blazephp], [php_BLAZEPHP.h])
